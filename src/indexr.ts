...
import * as elasticlunr from './index';
...
export class Index {
  ...
  static load(serialisedData: any) {
    if (serialisedData.version !== elasticlunr.version) {
      utils.warn('version mismatch: current '
    ...
    idx.documentStore = DocumentStore.load(serialisedData.documentStore);
    idx.pipeline = Pipeline.load(serialisedData.pipeline);
    idx.index = {};
    for (var field in serialisedData.index) {
      idx.index[field] = InvertedIndex.load(serialisedData.index[field]);
    }
 ...
 */
  ...
  addDoc(doc, emitEvent) {
    ...
    this._fields.forEach(function (field) {
      var fieldTokens = this.pipeline.run(tokenizer(doc[field]));
   ...
   * @memberOf Index
   */
  search(query?: string | {[index: string]: string}, userConfig?: any) {
    if (!query) return [];
    ...
    for (var i = 0; i < queryFields.length; i++) {
      ...
      queryTokens[key] = this.pipeline.run(tokenizer(query[key]));
    }
