...
export class Pipeline {
   ...
   * @memberOf Pipeline
   */
  static registerFunction (fn: Function, label: string) {
   ...
   * @memberOf Pipeline
   */
  static getRegisteredFunction (label: string) {
   ...
   * @memberOf Pipeline
   */
  static warnIfFunctionNotRegistered (fn: Function) {
    var isRegistered = (fn as any).label && ((fn as any).label in this.registeredFunctions);
  
    if (!isRegistered) {
      utils.warn('Function is not registered with pipeline. This may cause problems when serialising the index.\n', fn);
    }
  }
   ...
   * @memberOf Pipeline
   */
  after (existingFn: Function, newFn: Function) {
   ...
   * @memberOf Pipeline
   */
  before (existingFn: Function, newFn: Function) {
    Pipeline.warnIfFunctionNotRegistered(newFn);
   ...
   */
  toJSON () {
    return this._queue.map((fn) => {
      Pipeline.warnIfFunctionNotRegistered(fn);
      return fn.label as string;
    });
  }
}
